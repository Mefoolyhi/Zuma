...
def handle_embedding(balls_on_map, ball, last_ball_in_root,
    ...
    for i in range(len(balls_on_map)):
        ...
        if (last_ball_in_root is not None and ball == last_ball_in_root[i] or
                last_ball_in_root is None and
                ball == core_classes.Ball(enters[i][1], enters[i][0])):
            balls_on_map[i].append(copy.copy(ball))
            return i, n

        if (next_ball_in_root is not None and
                ball == core_classes.Ball(next_ball_in_root[i][1],
                                          next_ball_in_root[i][0]) or
                next_ball_in_root is None and ball == core_classes.Ball(
                    exits[i][1], exits[i][0])):
    ...
    while True:
        ...
        if line and len(line) > 0:
            ...
            if len(line) > 0:
                ...
                root, index = handle_embedding(
                    level._balls_on_map,
                    flying_ball,
                    last_ball_in_root,
                    next_ball_in_root_with_speed, level.enters, level.exits)
                score_delta = count_scores(level._balls_on_map, root, index)
                if root is None:
                    ...
                    line = []
                    if index == 0:
                        next_ball_in_root_with_speed[root] = next(level._iters[
                                                                      root],
                                                                  None)
                    score += score_delta
                    if score_delta > 0:
